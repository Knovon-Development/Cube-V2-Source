      ...
      if (Array.isArray(checkProduct)) {
        ...
        if (!productExists) {
          const embed = util.getEmbedTemplate(
            client,
            "error",
            "The product you have enetered does not exist. Are you even thinking properly?"
          );
        ...
        const embed = util.getEmbedTemplate(
          client,
          "error",
          "This hub doesn't have any products! Create one through `/createproduct` (C'mon man give my existance a purpose)."
        );
        return interaction.followUp({ embeds: [embed], ephemeral: true });
      }
    } catch (e) {
...
module.exports.info = {
  name: "licenserevoke",
  description: "Revoke a users license.",
  options: [
{
          type: 6,
          name: "user",
